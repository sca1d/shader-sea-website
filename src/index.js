...
const animate = () => {
  ...
  // カメラを円周上に周回させる
  camera.position.z = 3.0 * Math.cos(elapsedTime * 0.17);
  camera.position.x = 3.0 * Math.sin(elapsedTime * 0.17);

  camera.lookAt(Math.cos(elapsedTime), 0.5 * Math.sin(elapsedTime), 0.4 * Math.sin(elapsedTime));
